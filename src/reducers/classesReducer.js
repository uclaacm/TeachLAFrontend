...
let initialState = Map({ studClasses: Map(), instrClasses: Map() });
...
function classesReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case LOAD_STUDENT_CLASSES:
      ...
      return state.set("studClasses", Immutable.fromJS(action.classes));
    ...
    case LOAD_INSTR_CLASSES:
      // TODO: update this to make sure classes are indexed by class ID
      return state.set("instrClasses", Immutable.fromJS(action.classes));
